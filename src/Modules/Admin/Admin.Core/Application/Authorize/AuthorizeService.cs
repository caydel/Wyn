...
namespace Wyn.Mod.Admin.Core.Application.Authorize
{
    public class AuthorizeService : IAuthorizeService
    {
        ...
        public async Task<IResultModel> Login(LoginDto dto)
        {
            ...
            //查询账户
            var account = await _accountRepository.GetByUserName(dto.Username.DecodeBase64());
            if (account == null)
                return result.Failed("用户名或密码错误");

            //检测密码
            var password = _passwordHandler.Encrypt(dto.Password.EncodeBase64());
            ...
            {
                new(ClaimTypes.TENANT_ID, account.TenantId != null ? account.TenantId.ToString() : ""),
                new(ClaimTypes.ACCOUNT_ID, account.Id.ToString()),
                new(ClaimTypes.ACCOUNT_NAME, account.Name),
                new(ClaimTypes.PLATFORM, dto.Platform.ToInt().ToString()),
                new(ClaimTypes.LOGIN_TIME, dto.LoginTime.ToString())
            };

            //验证IP
            if (_authOptions.CurrentValue.EnableCheckIP)
            {
                claims.Add(new(ClaimTypes.IP, dto.IP));
            }
        ...
        public async Task<IResultModel> RefreshToken(RefreshTokenDto dto)
        {
            if (await _jwtTokenStorageProvider.Check(dto.RefreshToken, dto.AccountId, dto.Platform))
            {
                ...
                var claims = new List<Claim>
                {
                    ...
                    new(ClaimTypes.ACCOUNT_NAME, account.Name),
                    new(ClaimTypes.PLATFORM, dto.Platform.ToInt().ToString()),
                    new(ClaimTypes.LOGIN_TIME, DateTime.Now.ToTimestamp().ToString()),
                    new(ClaimTypes.IP, dto.IP)
                };
