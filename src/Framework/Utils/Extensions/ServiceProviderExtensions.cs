...
namespace Wyn.Utils.Extensions
{
    ...
    public static class ServiceProviderExtensions
    {
        ...
        public static T GetStartWith<T>(this IServiceProvider sp, string name) where T : class
        {
            ...
            return services.FirstOrDefault(m => m.GetType().Name.ToString().StartsWith(name, StringComparison.OrdinalIgnoreCase));
        }
        ...
        {
            var services = sp.GetServices<T>();
            return services.FirstOrDefault(m => m.GetType().Name.ToString().EndsWith(name, StringComparison.OrdinalIgnoreCase));
        }
    }
}
