...
namespace Wyn.Data.Core.Internal.QueryStructure
{
    /// <summary>
    /// 查询主体信息
    /// </summary>
    public class QueryBody
    {
        ...
        public string GetColumnName(Expression expression)
        {
            ...
            // 只有一个实体的时候，不需要别名
            return Joins.Count == 1 ? _dbAdapter.AppendQuote(columnName) : $"{join.Item1.Alias}.{_dbAdapter.AppendQuote(columnName)}";
        }
        ...
        public string GetColumnName(QueryJoin join, IColumnDescriptor descriptor)
        {
            ...
            // 只有一个实体的时候，不需要别名
            return Joins.Count == 1 ? _dbAdapter.AppendQuote(columnName) : $"{join.Alias}.{_dbAdapter.AppendQuote(columnName)}";
        ...
        public IColumnDescriptor GetColumnDescriptor(string name, QueryJoin join)
        {
            ...
            GenericHelper.NotNull(col, nameof(col), $"({name})列不存在");
        ...
        public IEntityDescriptor GetEntityDescriptor<TEntity>() where TEntity : IEntity 
            => Repository.DbContext.EntityDescriptors.First(m => m.EntityType == typeof(TEntity));
    }
}
