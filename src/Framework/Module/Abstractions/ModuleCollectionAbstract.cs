...
namespace Wyn.Module
{
    public abstract class ModuleCollectionAbstract : CollectionAbstract<IModuleDescriptor>, IModuleCollection
    {
        ...
        {
            if(assemblyDescriptor.Domain.IsNull())
                throw new ArgumentNullException($"{moduleDescriptor.Name}({moduleDescriptor.Code})模块的Domain程序集未发现",
                    $"{moduleDescriptor.Name}({moduleDescriptor.Code})模块的Domain程序集未发现");

            if (assemblyDescriptor.Infrastructure.IsNull())
                throw new ArgumentNullException($"{moduleDescriptor.Name}({moduleDescriptor.Code})模块的Domain程序集未发现",
                    $"{moduleDescriptor.Name}({moduleDescriptor.Code})模块的Infrastructure程序集未发现");

            if (assemblyDescriptor.Application.IsNull())
                throw new ArgumentNullException($"{moduleDescriptor.Name}({moduleDescriptor.Code})模块的Domain程序集未发现",
                    $"{moduleDescriptor.Name}({moduleDescriptor.Code})模块的Application程序集未发现");
        }
