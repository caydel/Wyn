...
namespace Wyn.Module.Core
{
    ...
    public class ModuleCollection : CollectionAbstract<ModuleDescriptor>, IModuleCollection
    {
        ...
        {
            var modulesRootPath = Path.Combine(AppContext.BaseDirectory, Constants.Constants.ROOT_DIR);
        ...
        {
            var jsonFilePath = Path.Combine(modulePath, Constants.Constants.JSON_FILE_NAME);
        ...
        private void LoadLayerAssemblies(ModuleDescriptor descriptor)
        {
            ...
            layer.Core = AssemblyHelper.LoadByNameEndString($"{Constants.Constants.PREFIX}.Mod.{descriptor.Code}.Core");
            layer.Web = AssemblyHelper.LoadByNameEndString($"{Constants.Constants.PREFIX}.Mod.{descriptor.Code}.Web");
            layer.Api = AssemblyHelper.LoadByNameEndString($"{Constants.Constants.PREFIX}.Mod.{descriptor.Code}.Api");
            layer.Client = AssemblyHelper.LoadByNameEndString($"{Constants.Constants.PREFIX}.Mod.{descriptor.Code}.Client");
        }
